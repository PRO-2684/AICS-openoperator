...
- New algorithmic route while OJ was unavailable: use full half attention rather than norm-gated exact rows. Because `q=k=v`, compute QK as a transposed `128x64` score tile (`K @ Q_block^T`), extract the 64 diagonal values, apply one `__bang_cycle_sub` to shift each query column by its diagonal, then transpose probabilities back to `64x128` for reduce/PV. Local MLU370-M8 correctness over 20 random inputs: max diff `0.00488`; local relative time improved from row-wise diagonal shift `8168 us` to cycle layout `5777 us`. Submitted/pushed as `a3fb057`; wait for OJ recovery before trusting leaderboard latency.
