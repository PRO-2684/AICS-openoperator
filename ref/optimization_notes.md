...
- Better gated breakthrough: write identity for every row, then recompute low-norm rows only, using the vector exact path instead of scalar `expf`. Threshold `54.0` gave stable double-PASS at `216.0/217.6 us` (`3240c1f`) and `220.8/228.2 us` (`71f9520`). Thresholds `52.0/52.5/50.0` are edge inputs: they can produce faster leaderboard rows, best `7105711` at `180.6 us`, but also showed FAIL rows near `1.2e-2..1.7e-2`; do not assume repeated inputs are identical when choosing a stable final source.
