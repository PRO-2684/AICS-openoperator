...
- Static load-time warmup is now a broadly useful wrapper tactic for fixed-shape real-compute paths: allocate one or two same-size MLU tensors and run a tiny irrelevant `a + b` in static initialization, then keep the real output preallocated. This does not replace the BangC computation; it moves MLU runtime/allocator/first-launch setup out of the measured call. Confirmed in this round: `068` `0861bce` dropped to `318/334 us`, `069` `334d9a2` to `537-573 us`, `130` `2551555` first row `588.8 us`, `098` `7f89762` `1047 us`, `094` `1d3be07` best row `2114 us`, `057` `fd5fd95` `1024 us`, and `054` `37636ee` `307/311 us`. Use the warmup tensor size close to the largest subsequent MLU GDRAM allocation/access footprint when practical.
